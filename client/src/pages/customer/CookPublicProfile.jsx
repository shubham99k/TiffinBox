      ...
      <div className='dashboard-content'>
        ...
        <div className='table-card' style={{ marginBottom: '24px' }}>
          <div style={{ padding: '28px 24px', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
            {cook?.photo
              ? <img src={cook.photo} alt='cook'
                  style={{ width: '100px', height: '100px', borderRadius: '16px', objectFit: 'cover', flexShrink: 0 }} />
              : <div style={{ width: '100px', height: '100px', borderRadius: '16px', background: 'var(--brand-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', flexShrink: 0 }}>
                  👩‍🍳
                </div>
            }
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '22px', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.5px', marginBottom: '4px' }}>
                {cook?.userId?.name}
              </div>
              <div style={{ fontSize: '13px', color: 'var(--subtle)', marginBottom: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <div className='cook-card-tags'>
                  {cook?.cuisineType?.map((c, i) => (
                    <span key={i} className='cook-tag'>{c}</span>
                  ))}
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {menu.dishes?.map((dish, i) => (
                  <div key={i} className='menu-dish-card'>
                    {dish.photo && (
                      <img src={dish.photo} alt={dish.name} className='menu-dish-image' />
                    )}
                    <div className='menu-dish-body'>
                      ...
                      <div className='menu-dish-footer'>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <div className='menu-dish-portions'>{dish.portionsLeft} left</div>
                          {dish.portionsLeft > 0 && (
                            <button
                              onClick={() => navigate('/orders/place', {
                                state: { menuId: menu._id, dishIndex: i, dish }
                              })}
                              style={{
        ...
        {reviews.length > 0 && (
          <div style={{ marginTop: '24px' }}>
            <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--ink)', marginBottom: '14px' }}>
